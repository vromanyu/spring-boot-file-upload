...
public class CustomExceptionsHandler {
    ...
    public RestResponse<ProblemResponse> handleValidationException(ConstraintViolationException e, HttpServerRequest request) {
        ...
        return RestResponse.status(RestResponse.Status.BAD_REQUEST,
                new ProblemResponse(request.uri(), errorMessages, OffsetDateTime.now(ZoneOffset.UTC), RestResponse.Status.BAD_REQUEST.getStatusCode()));
    }
    ...
    public RestResponse<ProblemResponse> handleFileNameNotMatchException(FileNameNotMatchException e, HttpServerRequest request) {
        return RestResponse.status(RestResponse.Status.BAD_REQUEST,
                new ProblemResponse(request.uri(), Collections.singletonList(e.getMessage()), OffsetDateTime.now(ZoneOffset.UTC), RestResponse.Status.BAD_REQUEST.getStatusCode()));
    }

    @ServerExceptionMapper
    public RestResponse<ProblemResponse> handleFileNotFoundException(FileNotFoundException e, HttpServerRequest request) {
        return RestResponse.status(RestResponse.Status.BAD_REQUEST,
                new ProblemResponse(request.uri(), Collections.singletonList(e.getMessage()), OffsetDateTime.now(ZoneOffset.UTC), RestResponse.Status.BAD_REQUEST.getStatusCode()));
    }
}
